%------------------------------------------------------------------
% JudgeNewRewardSequence class
%
%------------------------------------------------------------------

classdef JudgeNewRewardSequence < Screen
    ...
    properties (Constant)
        ...
        %defaultButtonLabelBasePosition = [.28 .22];
        defaultQuestionPosition = [.25 .32 .45 .06];
    ...
    methods
        % Constructor
        function vs = JudgeNewRewardSequence(figureHandle, playerName, symbols, sequence, outcomes, rewards, machineVersion)
        ...
        % machine
        function addQuestionButtons(vs, position, explainBoxPosition, submitButtonPosition)
        % Make the button area
            ...
            vs.questionSubmitButton = uicontrol('units','normalized', ...
            ...
            axes(vs.axesHandle);
            x1 = explainBoxPosition(1)-0.04;
            y1 = explainBoxPosition(2)-0.02;
            x2 = x1;
            y2 = explainBoxPosition(2)+explainBoxPosition(4)+0.04;
            ...
            eqText = 'Please explain your judgment.';
...
%                     % error
%             end
%             rText = sprintf('%s picked the %s. Click Next to continue.', vs.playerName, shape);
...
%                 switch r{playerbox}
%                     case MachineConstants.SQUARE
%                         vs.addSquare(vs.boxAxes(playerbox), MachineConstants.bigMachineFontSize, 1)
%                     case MachineConstants.TRIANGLE
%                         vs.addTriangle(vs.boxAxes(playerbox), MachineConstants.bigMachineFontSize, 1)
%                 end
%             % Copy machine
%             else
%                 playerbox = MachineConstants.PLAYERBOX;
...
%                 switch r{machinebox}
%                     case MachineConstants.SQUARE
%                         vs.addSquare(vs.boxAxes(machinebox), MachineConstants.bigMachineFontSize, 0)
%                     case MachineConstants.TRIANGLE
%                         vs.addTriangle(vs.boxAxes(machinebox), MachineConstants.bigMachineFontSize, 0)
%                 end
%             end
%             
...
%         end
        ...
        function showNextRound(vs, src, data)
            ...
            set(vs.questionSubmitButton, 'string', 'Submit', 'callback', @vs.submitJudgment);
        ...
        function submitJudgment(vs, src, data)
            ...
            vs.judgmentExplanations{vs.currRound} = explainText;
        ...
        function addInstructions(vs, position)
            ...
            instructions = {sprintf('Given the values of the colored chips shown on the left, please predict what %s will do on this round.', vs.playerName), '', sprintf('The machine on this cruise chip is a %s.', m)};
...
%---------- Main event loop ------------------------------------------------------------
        ...
        function show(vs)
            ...
            vs.addQuestionButtons(vs.defaultButtonSetPosition, vs.defaultExplainBoxPosition, ...
